...
<body>
    ...
    <section class="p-5">
        <div class="container">
            ...
            @include('message')
            <form class="form-section" method="POST" action="{{route('task.update', $task->id)}}">
                @csrf
                @method('PUT')
                <div class="mb-3">
                  <label for="title" class="form-label">Task Title</label>
                  <input class="form-control" type="text" placeholder="Example: Weekly Report" aria-label="default input example" id="title" value="{{$task->task_title}}" name="task_title">
                </div>
                <div class="mb-3">
                    <label for="description" class="form-label">Description</label>
                    <textarea class="form-control" type="text" placeholder="" aria-label="default input example" rows="5" name="description">{{$task->description}}</textarea>
                  </div>
